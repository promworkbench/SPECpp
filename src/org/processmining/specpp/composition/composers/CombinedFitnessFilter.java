...
public class CombinedFitnessFilter<I extends CompositionComponent<Place>, R extends Result> extends AbstractActivityActivationAwareFitnessFilter<I, R> {
    public CombinedFitnessFilter(ComposerComponent<Place, I, R> childComposer) {
        super(childComposer);
    }
    ...
    public void accept(Place place) {
        ...
        BasicFitnessEvaluation basicFitnessEvaluation = eval.getFractionalEvaluation();
        ...
        double relUnderfed = basicFitnessEvaluation.getRelativeUnderfedFraction();
        double combUnderfed = Math.min(aggUnderfed, relUnderfed);
        if (FitnessThresholder.isUnderfed(combUnderfed, thresholds)) {
        ...
        double relOverfed = basicFitnessEvaluation.getRelativeOverfedFraction();
        double combOverfed = Math.min(aggOverfed, relOverfed);
        if (FitnessThresholder.isOverfed(combOverfed, thresholds)) {
        ...
        double relFitness = basicFitnessEvaluation.getRelativeFittingFraction();
        double combFitness= Math.min(aggFitness, relFitness);
        if (FitnessThresholder.isTauFitting(combFitness, thresholds)) {
